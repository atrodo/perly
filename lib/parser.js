...
function Rule(atoms, sym)
{
  ...
  this.match = function(input)
  {
    ...
    console.log(tmp.indent + "+Rule.matchin " + this.atoms);
    for (var i in this.atoms)
    {
      var atom = this.atoms[i];
      console.log(tmp.indent + "+Rule.match on " + atom, atom.sym, tmp.is_left_recursive(atom.sym), tmp.last_sym(atom.sym), '-' + tmp.short);
      ...
      if (i == 0 && atom.sym != null)
      {
        ...
        if (is_left_rec)
        {
          ...
          {
            console.log(tmp.indent + "+Rule.match cont " + atom, tmp.is_left_recursive(atom.sym), tmp.last_sym(atom.sym));
            return_matches = false;
            continue;
          }

          console.log(tmp.indent + "+Rule.match false " + atom, tmp.is_left_recursive(atom.sym), tmp.last_sym(atom.sym));
          return;
        }
      }
      ...
      if (n == null)
      {
        console.log(tmp.indent + "+Rule.match end " + atom + " on '" + tmp.short + "'");
        console.log(tmp.indent + "+Rule.match end " + n);
        return;
      }
      ...
      tmp.matched(n);
      console.log(tmp.indent + "+Rule.matched " + n + " '" + tmp.short);
      return_matches = true;
    }

    if (!return_matches)
    {
      console.log(tmp.indent + "+Rule.!return_matches " + tmp.matches);
      return;
    }
...
Input.prototype.last_sym = function(sym)
{
  ...
  while (src != undefined )
  {
    ...
    */
  console.log(this.indent + 'M: false');
  return false;
}
...
Input.prototype._match_rules = function(rules, is_left_rec)
{
  for (var i in rules)
  {
    ...
    if (n != null)
    {
      ...
      console.log(this.indent + "-Yep " + rule + " " + " +(" + n + ")=(" + this_len +")(" + this._len_of(n) + ") - " + this.short + ' == ');
      //this.matched(n);
    ...
    console.log(this.indent + "-Noo " + rule + " on '" + this.short + "'");
  }
}
Input.prototype.parse = function(sym)
{
  ...
  {
    console.log(this.indent + "Found memory for " + memory_key + " (" + this.memory[memory_key] + ") on '" + this.short + "' ");
    return this.memory[memory_key];
  }
  console.log(this.indent + "Checked memory for " + memory_key + " (" + result + ") on '" + this.short + "' ");
  ...
  console.log(this.indent + "Attempting descent for " + memory_key + " on '" + tmp.short + "' " + tmp.matches);

  result = this._match_rules(rules, false);

  if (result == null)
  {
    console.log(this.indent + "-Nope " + rules + " " + memory_key + " - " + this.short);
  ...
  {
    var last_index = -1;
    console.log(this.indent + 'RECURSIVE');
    tmp.matched(result);
    tmp.matched(new SymMatched(sym, 0));

    while (last_index != tmp.index)
    {
      console.log(this.indent + "Attempting recursion for " + memory_key + " on '" + tmp.short + "' " + tmp.matches);
    ...
    result = tmp.matches;
    console.log(this.indent + 'Finlen: ' + this._len_of(result) + ' : '  + tmp.short);
    /*
  ...
  console.log(this.indent + "Saved memory for " + memory_key + " (" + result + ") on '" + this.short + "' ");
  this.memory[memory_key] = result;
  return result;
}
