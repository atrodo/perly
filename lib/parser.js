...
var SymMatched = function(sym, length)
{
  ...
  {
    console.trace();
    throw "SymMatched requires an integer length, not " + length;
  }
  this.len = length;
  this.sym = sym;
}
...
{
  var genmem = {};
  var args = { length: this.length, snode: this, genmem: genmem, getters: {} };
  var items = this;
...
var Input = function(src, nodes, options)
{
  var index = 0;
  var indent = "";
  var memory = _.merge({ max_length: 0, lex: {}, recursive_sym: null }, options);
...
module.exports = function(start, gmr)
{
        ...
        function(rule, i)
        {
          ...
          if ( rule instanceof Array )
          {
            ...
            _.each(rule,
              function(item)
              {
                if (GNode.isPrototypeOf(item))
                {
                  gnode_items.unshift(item);
                }
                else
                {
                  rule_items.unshift(item);
                }
              }
            );
            ...
            _.each(rule_items,
              function(item)
              {
                var combined = _.concat([], item, gnode_items);
                nodes[sym].push(new Rule(combined, sym));
              }
            );
            return;
          }

          nodes[sym].push(new Rule(rule, sym));
        }
      );
  ...
  var parser = function(source, options)
  {
    ...
    var input = new Input(source, nodes, options);
  ...
  var eval_source = function(source, runtime, options)
  {
    if (!(source instanceof SNode))
    {
      source = parser(source, options);
    }
